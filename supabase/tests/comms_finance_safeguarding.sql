...
reset role;
rollback to savepoint guardian_consent;

savepoint sensitive_reads;
set local role authenticated;
...
rollback to savepoint sensitive_reads;

savepoint finance_flow;
set local role authenticated;
select set_config('request.jwt.claim.sub','00000000-0000-4000-8000-000000000203',true);
...
reset role;
rollback to savepoint finance_flow;

savepoint webhook_flow;
...
rollback to savepoint webhook_flow;

savepoint stale_job_recovery;
...
rollback to savepoint stale_job_recovery;

savepoint account_erasure;
...
rollback to savepoint account_erasure;

savepoint deletion_holds;
...
rollback to savepoint deletion_holds;
